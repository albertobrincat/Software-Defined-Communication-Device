...
class Controller(tornado.websocket.WebSocketHandler):
    ...
    ip_address1='' #IP ADDRESS OF MACHINE WITH GNURADIO TX FILES
    ip_address2='' #IP ADDRESS OF MACHINE WITH RX CONTROLLER
    ...
    def parser_message(self,message):
        ...
        payload="message:'"+x.message+"', seq:'"+str(x.counter)+"', sent:'"+x.timestamp+"'"
	    ...
	    if self.p.is_alive()==False :
               ...
               self.p.start()
            time.sleep(2)
            self.socket_tcp_client(payload,8001,self.ip_address1)
	    ...
	    if self.p1.is_alive()==False :
               ...
               self.p1.start()
            time.sleep(2)
            self.socket_tcp_client(payload,8002,self.ip_address1)
    ...
    def socket_tcp_client(self,message,port,ip_address):
        ...
        try:
             ...
             #Send data
             print "SOCKET TCP SENDING--->%s" %message
